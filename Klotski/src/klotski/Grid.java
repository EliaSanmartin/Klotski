...
public class Grid 
{
	...
	protected final int righe = 5;
	protected final int colonne = 4;
	
	//-------Posizione del mouse------
    protected short victory = 0;
    private int gridSize = 100;//dimensione quandrato in javafx
	
    protected int id_old = 0;//in questa variabile viene salvato l'id del pezzo mosso per sapere quando si muove un altro pezzo
    
    private int num_mosse = 0;//numero mosse
	
    protected int [][] mat = new int[righe][colonne];//Griglia che dice come sono posizionati i pezzi
	
    private Level l;//questa classe serve per sapere dove posizionare i pezzi e quali in base al livello
	
    protected ArrayList<GeometricShape> rectangles = new ArrayList<>();//pezzi per la parte grafica
    protected List<Shape> pezzi = new ArrayList<>();//pezzi per la parte dei controlli
    protected List<Mossa> mosse = new ArrayList<>();//tutte le mosse fatte fino ad ora per tornare indietro e per salvarle
	
    protected String nome_file;//dove verrà salvato il file
	
    protected FileS salvataggio;//in questa classe verranno salvate le mosse e le aggiungerà ad un file
	...
	public void next_move() throws IOException
	{
		...
		if(m.dir == 'N')//se non l'ha trovata
		{
		...
		m.num_pezzo = mat[m.c_in.getX()][m.c_in.getY()];//seleziono il pezzo da muovere
		...
		r = rectangles.get(m.num_pezzo - 1);//trovo quadrato da muovere
		node = r.getRectangle();
		//-----sposto quadrato-----
		
		node.setLayoutX(m.c_fin.getY()*gridSize - r.getX());
		node.setLayoutY(m.c_fin.getX()*gridSize - r.getY());
		if(id_old != r.getID() - 1)//se si cambia il blocco spostato
    	{
    		id_old = r.getID() - 1;//segna il blocco nuovo che è stato spostato
			add_move();
    	}
		victory = pezzi.get(m.num_pezzo-1).imposta_coord(m, mat);//fai la mossa 
		if (victory == 1)
		{
			victory = 2;
			victory();
	...
	public void undo()
	{
		if(mosse.size() != 0)
		{
			...
			//-----esegui mossa-----
			victory = pezzi.get(m.num_pezzo-1).imposta_coord(m, mat);
	...
	public void resumegames(FileS file)
	{
		...
		for(int i=1;i<file.contenuto.size();i++)
		{			
